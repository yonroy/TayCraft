---
name: phieu-canvas
description: Dựng/đổi một phiếu "AI by Hand" theo phong cách CANVAS TOÀN CẢNH (Tom Yeh) — A4 ngang, sơ đồ kiến trúc bên trái + bảng số bên phải, điền ô trắng, có nút 🎲 và trang đáp án. Dùng khi tạo phiếu kiến trúc/khái niệm mới, hoặc chuyển phiếu cũ sang phong cách canvas. Đây là HOUSE STYLE đã chốt cho bộ phiếu.
---

# Phiếu CANVAS TOÀN CẢNH ("AI by Hand" — kiểu Tom Yeh)
...
<title>AI by Hand · <Tên> — canvas (A4 ngang)</title>
...
<section class="sheet">            <!-- TRANG ĐỀ -->
  ...
  <div class="foot"><span>AI by Hand ✍️ · Tên</span><span>Trang 1/2 · ĐỀ</span></div>
</section>
...
2. **Đo tràn BẰNG SỐ** (mỗi `.sheet` `overflow:hidden` → tràn bị cắt âm thầm). Chèn vào bản sao `_m.html` rồi chạy Edge headless `--dump-dom`, đọc thuộc tính:
   ```js
   window.addEventListener('load',()=>setTimeout(()=>{var o='';document.querySelectorAll('.sheet').forEach((p,i)=>{var ov=p.scrollHeight-p.clientHeight;o+='T'+(i+1)+'='+ov+(ov>2?'!TRAN ':' ');});document.documentElement.setAttribute('data-measure',o);},700));
   ```
   Đo **≥3 lần** (số ngẫu nhiên rộng làm wrap đổi). A4 ngang: chiều cao nội dung ≤ **~165mm/sheet**; vượt thì **tách thêm sheet** (footer `X/N`), đừng nhồi.
   Edge: `"…/msedge.exe" --headless=new --disable-gpu --virtual-time-budget=4000 --dump-dom "file:///…/NN_m.html"`
