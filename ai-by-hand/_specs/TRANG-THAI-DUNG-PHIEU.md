...
> Cập nhật cuối: 2026-06-28 (#8) — **8 phiếu K2 CŨ pre-spec → dựng lại theo spec GIẢNG GIẢI → KHÓA K2 ĐỒNG BỘ HOÀN TOÀN.** Dựng lại D3 cross-entropy, D5 gradient, D7 backpropagation, D13 adam-một-bước, E7 layernorm/rmsnorm, F1 cnn-tích-chập (chuyển **A3 inline → A4 `wb.css`**), G2 rnn-một-bước, G4 lstm-một-ô-nhớ. Các file trước đã là giảng-giải nhưng thiếu `.intuition` + `.quiz` (+ rút-ra trỏ sai); nay đủ 5 lớp (intro/intuition/why/quiz/rút-ra) + ≥1 SVG đúng bản chất, đo tràn 0px ×3–5, de-key sạch (8/8 PASS), soát ảnh ✅; ghi đè cùng slug → KHÔNG đụng lessons.ts/index.html. Sửa: D13 v₁ hiện 3 chữ số (0.016/0.001=16 khớp); F1 feature-map ĐỀ để trống cả 4 ô (không lộ đáp án bước 1–2). **→ Toàn bộ K2 (D+E+F+G) nay GIẢNG GIẢI thống nhất, không còn phiếu canvas/pre-spec/A3.**
> Cập nhật cuối: 2026-06-28 (#7) — **K2 Phần F + G HOÀN TẤT (giảng-giải, 15/15) → KHÓA K2 ĐỒNG BỘ GIẢNG-GIẢI TRỌN D+E+F+G (35 phiếu).** Chuyển canvas→giảng-giải: **F** (9): F2 kích-thước-đầu-ra, F3 conv-đa-kênh, F4 nhiều-bộ-lọc, F5 pooling, F6 receptive-field, F7 conv-1×1, F8 transposed-conv, F9 residual, F10 đếm-tham-số. **G** (6): G3 BPTT, G5 GRU, G6 BiRNN, G7 seq2seq, G8 greedy/beam, G9 teacher-forcing. Mỗi phiếu ≥1 SVG đúng bản chất, đo tràn 0px ×3–6, de-key sạch (15/15 PASS), soát ảnh ✅; ghi đè canvas cùng slug → KHÔNG đụng lessons.ts/index.html. Sửa bug số học G8 (greedy bước-2 phải = AA: chỉ giữ preset p2aa≥p2ab & beamBest≠AA). 8 phiếu K2 CŨ pre-spec (D3,D5,D7,D13,E7,F1,G2,G4) vẫn để nguyên (ngoài phạm vi đợt này).
